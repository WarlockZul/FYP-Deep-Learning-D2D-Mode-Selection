...
def main():
    ...
    callbacks = [
        ...
        ModelCheckpoint("models/cnn/cnn_model.keras", monitor='val_mae', save_best_only=True),
        CSVLogger("models/cnn/cnn_training_log.csv", separator=',', append=False)
    ]
    ...
    print("\nTraining Complete. Best model saved to 'models/cnn/cnn_model.keras'")
...
def plot_training_history(history):
    ...
    plt.title('CNN: Mean Squared Error (Loss)')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig("models/cnn/cnn_training_curve.png", dpi=300) 
    plt.show()
